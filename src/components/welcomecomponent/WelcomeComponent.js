...
import { KEY_ID, KEY_SECRET } from '../../razorpay'
const pay = (order) => {
    ...
    let options = {
        ...
        "handler": function (response) {
            swal({
                ...
                text: `Plan valid till ${format(new Date(addMonths(new Date(), 1)), 'do-MMM-yyyy')}`,
                icon: "success",
            });
                ...
                <div className=" col_text col-md-5">
                    <div className="d-flex mx-auto">
                        <div className="text__wrapper">
                            ...
                            <button onClick={() => paymentOrder()} className="btn btn-primary join_prime ">Get Started</button>
                        </div>
                    </div>
                </div>
